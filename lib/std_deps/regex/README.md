...
| Unicode | `\xHH`, `\x{...}`, `\N{name}`, `\p{...}`, `\P{...}`, pinned general-category aliases, and scalar `ASCII`, `Cased`, `Lowercase`, `Uppercase`, `Alphabetic`, `White_Space`, `Hex_Digit`, `Math`, and `Currency_Symbol` binary properties | Implemented; broader binary/script/Bidi tables remain planned |
