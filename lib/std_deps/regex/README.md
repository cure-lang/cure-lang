...
| Unicode | `\xHH`, `\x{...}`, `\N`, `\N{name}`, `\p{...}`, `\P{...}`, pinned general-category aliases including `General_Category=...`/`gc=...` and derived names such as `Assigned`, all boolean properties in the pinned Unicode property table (including `Emoji`, `Extended_Pictographic`, and `ID_Start`), scalar `ASCII`, `Cased`, `Lowercase`, `Uppercase`, `Alphabetic`, `White_Space`, `Hex_Digit`, `Math`, and `Currency_Symbol` binary properties, all names in the pinned Unicode Script table including `Script=...`/`sc=...`, Script_Extensions including `Script_Extensions=...`/`scx=...`, `Bidi_Class`/`bc` short and long values, `Bidi_Mirrored`, and `Bidi_Paired_Bracket_Type`/`bpt` | Implemented; other non-finite Bidi-value properties remain planned |
