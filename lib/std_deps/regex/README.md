...
| Unicode | `\xHH`, `\x{...}`, `\N{name}`, `\p{...}`, `\P{...}`, pinned general-category aliases, all boolean properties in the pinned Unicode property table (including `Emoji`, `Extended_Pictographic`, and `ID_Start`), scalar `ASCII`, `Cased`, `Lowercase`, `Uppercase`, `Alphabetic`, `White_Space`, `Hex_Digit`, `Math`, and `Currency_Symbol` binary properties, all names in the pinned Unicode Script table including `Script=...`/`sc=...`, and `Bidi_Class`/`bc` short and long values | Implemented; script extensions and other Bidi-value properties remain planned |
