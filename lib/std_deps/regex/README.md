...
| Unicode | `\xHH`, `\x{...}`, `\N{name}`, `\p{...}`, `\P{...}`, pinned general-category aliases, and scalar `ASCII`, `Cased`, `Lowercase`, and `Uppercase` binary properties | Implemented; broader binary/script/Bidi tables remain planned |
