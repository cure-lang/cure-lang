defmodule Antigen.Assays.ElabSoundnessTest do
  ...
  describe "fuel bound" do
    ...
    # assume elaboration prevents a non-normalizing emitted def, and must never
    # HANG on one, however the fuel-bounded conversion path resolves it.
    #
    ...
    #
    # `Normalise.unfold_certified_head` unfolds a BARE self-reference
    # (`Eval.eval({:global, :loop}, []) == {:vneutral, {:nglobal, :loop}}`) and
    # recognizes the result is the IDENTICAL neutral it started from — no
    # progress is possible, ever — so it freezes the application as stuck after
    # exactly one step (the "unchanged neutral is stuck" guard, 07006ad4, added
    # for indexed branch refinement). `probe`'s declared `Eq(Nat, loop, Z)` and
    # inferred `Eq(Nat, Z, Z)` are then decisively NOT convertible — a sound
    # answer, reached in O(1) rather than by exhausting the fuel budget. This is
    # why the body is a bare global reference and not a self-application:
    # `{:app, {:global, :loop}, {:ctor, :Z, []}}` would (a) make `loop`'s OWN
    # check_one infer fail structurally with `:not_a_function` BEFORE any δ (its
    # declared type `@nat` isn't a Π, so applying it is ill-typed — this never
    # reaches δ/fuel at all, the original bug in this test), and (b) even
    # redirected through `probe`, would re-grow the neutral's `:napp` spine by
    # one layer per δ-step — defeating the same-neutral guard (a growing spine is
    # never identical to the last one), making `spine/2`'s per-step traversal
    # O(step), i.e. O(fuel²) total — likely multiple minutes at fuel = 500_000,
    # blowing the 30s `Task.await` budget.
    test "non-normalizing emitted def is decided soundly and promptly, never a fuel-exhausted hang" do
      env =
        seeded()
      ...
      assert {:violation, {:core_ill_typed, :probe, {:conversion_failure, _actual, _expected}}} =
