defmodule Cure.Stdlib.DependentRegexAssertionDecisionTest do
  ...
  test "selected atomic starts retain their source-start suffix" do
    ...
    for constructor <- ["AtomicSelectedStartActive", "AtomicSelectedStartAccepted"] do
      line = Enum.find(String.split(source, "\n"), &String.contains?(&1, "#{constructor} :"))
      assert line =~ "start_selection_suffix"
    end
  end
  ...
  test "blocked atomic starts retain typed skip evidence" do
    ...
    assert source =~ "type AtomicStartSkipEvidence"
    assert source =~ "AtomicStartSkipByCommit"
    assert source =~ "AtomicStartSkipByScope"
    assert Regex.match?(~r/AtomicStartCandidateBlocked\s*:.*?AtomicStartSkipEvidence/s, source)
  ...
  test "active rejected starts consume their stored child suffix" do
    source = File.read!("lib/std_deps/regex/regex_runtime.cure")

    assert source =~ "fn atomic_start_candidate_rejection_excludes_trace"
    assert Regex.match?(~r/atomic_start_candidate_rejection_excludes_trace.*?AtomicPathRefutation/s, source)
    assert Regex.match?(~r/atomic_start_candidate_rejection_excludes_trace.*?AtomicSelectedStartActive/s, source)
  ...
  test "atomic path refutations carry a constructor kind index" do
    source = File.read!("lib/std_deps/regex/regex_runtime.cure")

    assert source =~ "type AtomicPathFailureKind"
    assert source =~ "AtomicPathFailureInputExhausted"
    assert source =~ "AtomicPathFailureExactAccepted"
    assert source =~ "AtomicPathFailureDestinationsExhausted"
    assert source =~ "AtomicPathFailureDestinationRejected"
  ...
  test "atomic path refutations consume non-empty exhausted tails" do
    source = File.read!("lib/std_deps/regex/regex_runtime.cure")
  ...
  test "atomic start skips exclude selected traces at the construction site" do
    ...
    assert Regex.match?(
             ~r/fn atomic_start_blocked_active_candidate_excludes_trace\b.*?AtomicStartFailureBlocked.*?AtomicSelectedStartActive.*?atomic_start_skip_excludes_allowed/s,
             source
           )

    assert Regex.match?(
             ~r/fn atomic_start_blocked_accepted_candidate_excludes_trace\b.*?AtomicStartFailureBlocked.*?AtomicSelectedStartAccepted.*?atomic_start_skip_excludes_allowed/s,
             source
           )
  end
  ...
  test "active rejected start evidence retains its indexed tail" do
    ...
    assert source =~ "type AtomicStartActiveRejectedEvidence"
    assert source =~ "fn atomic_start_active_rejected_evidence"
    assert Regex.match?(~r/atomic_start_active_rejected_evidence.*?AtomicStartActiveRejectedEvidencePacked/s, source)
  end

  test "rejected tail evidence retains its recursive refutation" do
    source = File.read!("lib/std_deps/regex/regex_runtime.cure")

    assert source =~ "type AtomicStartTailPackage"
  ...
  test "rejected start induction consumes the exact failure suffix" do
    source = File.read!("lib/std_deps/regex/regex_runtime.cure")
  ...
  test "rejected induction uses an erased tail proof thunk" do
    ...
    assert source =~ "fn atomic_start_rejected_member_induction"
    assert Regex.match?(~r/atomic_start_rejected_member_induction.*?AtomicStartMemberThere.*?tail_case/s, source)
  end

  test "rejected member induction has a construction-site head consumer" do
    source = File.read!("lib/std_deps/regex/regex_runtime.cure")
  ...
  test "non-empty selected membership has an erased tail-witness transport" do
    ...
    assert source =~ "fn atomic_selected_start_witness_tail"
    assert Regex.match?(~r/atomic_selected_start_witness_tail.*?ListMember/s, source)
    assert Regex.match?(~r/atomic_selected_start_witness_tail.*?AtomicSelectedStartWitnessPacked/s, source)
  end

  test "active rejected tails retain child scope alignment" do
    source = File.read!("lib/std_deps/regex/regex_runtime.cure")
  ...
  test "atomic destination rejection preserves recursive rejected-tail descent" do
    source = File.read!("lib/std_deps/regex/regex_runtime.cure")
  ...
  test "atomic destination rejection has a generic recursive-tail dispatcher" do
    ...
    assert source =~ "fn atomic_path_destination_rejection_excludes_recursive_tail"

    [_prefix, body] =
      String.split(source, "fn atomic_path_destination_rejection_excludes_recursive_tail(\n", parts: 2)

    [body | _] = String.split(body, "\n  ##", parts: 2)
    assert body =~ "remaining: List(LookaroundAdmittedState(n))"
    assert Regex.match?(~r/atomic_path_selected_child_suffix_location_select.*?head_case.*?tail_case/s, body)
  end
  ...
  test "rejected-tail construction sites use the generic dispatcher" do
    ...
    [_prefix, body] =
      String.split(source, "fn atomic_path_destination_rejection_excludes_recursive_tail_rejected(\n", parts: 2)

    [body | _] = String.split(body, "\n  ##", parts: 2)
    assert body =~ "atomic_path_destination_rejection_excludes_recursive_tail("
  end

  test "singleton rejected traces use the generic tail dispatcher" do
    source = File.read!("lib/std_deps/regex/regex_runtime.cure")
  ...
  test "destination exhaustion tails use the generic recursive dispatcher" do
    ...
    [_prefix, body] =
      String.split(source, "fn atomic_path_tail_destinations_exhausted_excludes_aligned_trace(", parts: 2)

    [body | _] = String.split(body, "\n  ##", parts: 2)
    assert body =~ "atomic_path_destination_rejection_excludes_recursive_tail("
  end

  test "accepted destination tails use the generic recursive dispatcher" do
    source = File.read!("lib/std_deps/regex/regex_runtime.cure")
