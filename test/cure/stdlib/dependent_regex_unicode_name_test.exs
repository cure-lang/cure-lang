defmodule Cure.Stdlib.DependentRegexUnicodeNameTest do
  ...
  setup_all do
    ...
    mod RegexUnicodeNames
      ...
      fn line_feed(input: String) -> Option(Unit) = parse_full(/\R/, input)
      fn carriage_return_line_feed(input: String) -> Option(Unit) = parse_full(/\R/, input)
      fn unicode_line_separator(input: String) -> Option(Unit) = parse_full(/\R/, input)
      fn lf_policy(input: String) -> Option(Unit) = parse_full(/(*LF)\R/, input)
      fn cr_policy(input: String) -> Option(Unit) = parse_full(/(*CR)\R/, input)
      fn crlf_policy(input: String) -> Option(Unit) = parse_full(/(*CRLF)\R/, input)
      fn anycrlf_policy(input: String) -> Option(Unit) = parse_full(/(*ANYCRLF)\R/, input)
      fn any_policy(input: String) -> Option(Unit) = parse_full(/(*ANY)\R/, input)
      fn bsr_anycrlf_policy(input: String) -> Option(Unit) = parse_full(/(*BSR_ANYCRLF)\R/, input)
      fn bsr_unicode_policy(input: String) -> Option(Unit) = parse_full(/(*BSR_UNICODE)\R/, input)
  ...
  test "R matches CRLF as one finite line-break alternative and Unicode separators", %{runtime_module: module} do
    assert apply(module, :line_feed, [{:String, [10]}]) == {:some, :unit}
    assert apply(module, :carriage_return_line_feed, [{:String, [13, 10]}]) == {:some, :unit}
    assert apply(module, :unicode_line_separator, [{:String, [0x2028]}]) == {:some, :unit}
  ...
  test "newline controls select finite R policies", %{runtime_module: module} do
    assert apply(module, :lf_policy, [{:String, [10]}]) == {:some, :unit}
    assert apply(module, :lf_policy, [{:String, [13]}]) == :none
    assert apply(module, :lf_policy, [{:String, [13, 10]}]) == :none

    assert apply(module, :cr_policy, [{:String, [13]}]) == {:some, :unit}
    assert apply(module, :cr_policy, [{:String, [10]}]) == :none

    assert apply(module, :crlf_policy, [{:String, [13, 10]}]) == {:some, :unit}
    assert apply(module, :crlf_policy, [{:String, [13]}]) == :none
    assert apply(module, :crlf_policy, [{:String, [10]}]) == :none

    assert apply(module, :anycrlf_policy, [{:String, [13]}]) == {:some, :unit}
    assert apply(module, :anycrlf_policy, [{:String, [10]}]) == {:some, :unit}
    assert apply(module, :anycrlf_policy, [{:String, [13, 10]}]) == {:some, :unit}
    assert apply(module, :anycrlf_policy, [{:String, [0x2028]}]) == :none

    assert apply(module, :any_policy, [{:String, [0x2028]}]) == {:some, :unit}
    assert apply(module, :bsr_anycrlf_policy, [{:String, [0x2028]}]) == :none
    assert apply(module, :bsr_unicode_policy, [{:String, [0x2028]}]) == {:some, :unit}
  end
