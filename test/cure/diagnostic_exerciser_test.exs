defmodule Cure.DiagnosticExerciserTest do
  ...
  test "exercises every public diagnostic family and shows user output" do
    compiler_cases = [
      ...
      {"binder pipe monad mismatch", "E121",
       "mod DiagnosticBindPipeMismatch\n  use Std.Monad\n  use Std.Result\n  use Std.Option\n  fn bad() -> Result(Int, Atom) = ok(1) |x|> Some(x)\nend\n",
       :bind_pipe_monad_mismatch},
      {"binder pipe without monad", "E122", "mod DiagnosticBindPipeNoMonad\n  fn bad() -> Int = 1 |x|> x + 1\nend\n",
       :bind_pipe_no_monad},
