...
| Non-newline class | `\N` | yes | yes | yes | Finite complement of Cure's Unicode newline predicate |
...
| Script extensions | `\p{scx=Hira}`, `\p{Script_Extensions=Hiragana}` | yes | yes | yes | Pinned Unicode 17.0.0 ScriptExtensions table with primary-Script fallback |
| Bidi properties | `\p{Bidi_Class=AL}`, `\p{Bidi_Mirrored}`, `\p{bpt=o}` | yes | yes | yes | Pinned Bidi_Class aliases, UnicodeData Bidi_Mirrored field, and Unicode 17.0.0 BidiBrackets Bidi_Paired_Bracket_Type table; other Bidi mapping properties remain planned |
